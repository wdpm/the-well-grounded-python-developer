...
from ..emailer import send_mail_by_mailtrap_testing as send_mail
import json
...
def login():
    ...
    if form.validate_on_submit():
        with db_session_manager() as db_session:
            ...
            #  url_parse(next).netloc != "" means NOT a relative path
...
# 这里的@login_required需要重新考虑，一般而言，点击激活确认链接，是直接激活的，而不是要求登录后来激活。
@auth_bp.get("/confirm/<confirmation_token>")
@login_required
def confirm(confirmation_token):
    """
    confirm example link: http://127.0.0.1:5000/confirm/eyJjb25maXJtIjoiZDg0ZDQwYjU5MzFlNDg0NjllNmFiMjcwOTllNzkzYTMifQ.ZSoQnw.G-jCinAIayRGNgqlTwnvGofdOHE
    """
    ...
    try:
        ...
        if current_user.confirm_token(confirmation_token):
            with db_session_manager() as db_session:
                # flask session.property, not db property name
                current_user.confirmation = True
                db_session.add(current_user)
                db_session.commit()
                flash("Thank you for confirming your account")
                logger.debug(f'Confirmation succeeded for current user {current_user}.')
        else:
            logger.debug(f'Confirmation failed for current user {current_user}.')
...
def request_reset_password():
    # why should check this condition?
    # 你都登录了，代表你肯定没有忘记密码，你不需要重置密码，再见
    if current_user.is_authenticated:
        return redirect("intro_bp.home")
